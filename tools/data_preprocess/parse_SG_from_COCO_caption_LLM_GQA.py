...
from maskrcnn_benchmark.data.datasets import GQADataset
from maskrcnn_benchmark.data.preprocess_utils import map_caption_concepts_to_gqa200_categories
...
gqa_test_dataset = GQADataset(split='test',
...
print(len(gqa_test_dataset))


# parse text scene graphs from captions

processed_coco_caption_file = 'dataset/GQA/aligned_triplet_info_gqa.json'
...
if os.path.exists(processed_coco_caption_file):
    ...
    caption_object_vocabs_to_vg_objs, caption_relation_vocabs_to_vg_rels = map_caption_concepts_to_gqa200_categories(
        list(caption_object_vocabs), list(caption_relation_vocabs), gqa_test_dataset
    )
    ...
    for index, (img_id, img_sg_info) in enumerate(tqdm(coco_parsing_info['text_scene_graph'].items())):
        ...
        for (s, p, o) in img_sg_info['triplets']:
            ...
            img_triplets.append({
                ...
                "vg150_predicate_label": gqa_test_dataset.ind_to_predicates.index(p),
                "text": (s, p, o)
            })
        ...
        for ent_id in range(len(entities)):
            ...
            img_entities.append({
                ...
                "vg150_obj_label": gqa_test_dataset.ind_to_classes.index(label_name),
