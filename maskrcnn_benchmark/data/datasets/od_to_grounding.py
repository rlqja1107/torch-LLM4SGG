...
def convert_od_to_grounding_simple_pred(
    target, 
    image_id, 
    ind_to_class, 
    ...
    rel_pair_idx = target.extra_fields['relation'].nonzero()
    relation_cls = target.extra_fields['relation'][rel_pair_idx[:,0], rel_pair_idx[:,1]]
    for i in range(len(relation_cls)):
        new_target_i = {}
        new_target_i["image_id"] = image_id
        new_target_i["category_id"] = relation_cls[i].item()
        new_target_i["id"] = None

        label_i = relation_cls[i].item()
