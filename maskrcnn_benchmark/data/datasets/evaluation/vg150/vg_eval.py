...
def do_vg_evaluation(
    ...
    zeroshot_triplet = torch.load(f"{cfg.DATASETS.ZEROSHOT_PATH}", map_location=torch.device("cpu")).long().numpy()
    #zeroshot_triplet = torch.load("maskrcnn_benchmark/data/datasets/evaluation/vg150/zeroshot_triplet_real.pytorch", map_location=torch.device("cpu")).long().numpy()
    ...
    if "relations" in iou_types or cfg.MODEL.DYHEAD.RELATION_CONSISTENCY_ON:
        ...
        if output_folder:
            with open(os.path.join(output_folder, "evaluation_res.txt"), 'w') as f:
                f.write(result_str)
    logger = setup_logger("maskrcnn_benchmark", output_folder, 0)
