...
# assign target label
def assign_tgtlabel_to_proposals(proposals, targets, matcher):
    for img_idx, (target, proposal) in enumerate(zip(targets, proposals)):
        ...
        # set predict label and target label
        # det_pred_labels = proposals[img_idx].get_field("labels").clone()
        # proposals[img_idx].add_field("pred_labels", det_pred_labels)
...
class RelationSampling(object):
    ...
    def prepare_test_pairs(self, device, proposals, MAX_PAIR_NUM=1000):
        ...
        for p in proposals:
            ...
            # limit size of relation pairs in prediction
            if len(idxs) > MAX_PAIR_NUM:
                coarse_pair_scores = p.get_field('scores')[idxs].prod(-1) # proposal score
                _, pinds = coarse_pair_scores.topk(MAX_PAIR_NUM)
                idxs = idxs[pinds]
