...
class PostProcessor(nn.Module):
    ...
    def forward(self, x, rel_pair_idxs, boxes):
        ...
        for i, (rel_logit, obj_logit, rel_pair_idx, box) in enumerate(zip(
            ...
            if self.use_gt_box or 'boxes_per_cls' not in box.extra_fields:
                obj_scores, obj_pred = obj_class_prob[:, 1:].max(dim=1)
                obj_pred = obj_pred + 1
            else:
                # NOTE: by , apply late nms for object prediction
            ...
            # TODO : how about using weighted some here?  e.g. rel*1 + obj *0.8 + obj*0.8
            ...
            # Note
            # TODO : add a new type of element, which can have different length with boxlist (similar to field, except that once 
