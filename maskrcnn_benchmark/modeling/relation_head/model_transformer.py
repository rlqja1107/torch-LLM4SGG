...
class TransformerContext(nn.Module):
    ...
    def forward(self, roi_features, proposals, logger=None):
        ...
        if self.cfg.MODEL.ROI_RELATION_HEAD.NO_OBJECT_REFINE:
            obj_preds = obj_labels
            obj_dists = to_onehot(obj_labels, self.num_obj_cls, fill=cat([proposal.get_field("scores") for proposal in proposals], dim=0).detach()*10)
            edge_pre_rep = self.lin_obj(obj_pre_rep)
        else:
            ...
            if self.mode == 'predcls':
                ...
                use_decoder_nms = self.mode == 'sgdet' and not self.training and ('boxes_per_cls' in proposals[0].extra_fields)
                if use_decoder_nms:
