...
class LSTMContext(nn.Module):
    ...
    def obj_ctx(self, obj_feats, proposals, obj_labels=None, boxes_per_cls=None, ctx_average=False):
        ...
        if self.cfg.MODEL.ROI_RELATION_HEAD.NO_OBJECT_REFINE:
            ...
            obj_dists = to_onehot(obj_labels, self.num_obj_classes, fill=cat([proposal.get_field("scores") for proposal in proposals], dim=0).detach()*10)
        else:
    ...
    def forward(self, x, proposals, rel_pair_idxs, logger=None, all_average=False, ctx_average=False):
        ...
        boxes_per_cls = None # ??
        if self.mode == 'sgdet' and not self.training and ('boxes_per_cls' in proposals[0].extra_fields):
