...
class Anchors(nn.Module):
    """
    adapted and modified from https://github.com/google/automl/blob/master/efficientdet/anchors.py 
    """
...
def display(preds, imgs, obj_list, imshow=True, imwrite=False):
    for i in range(len(imgs)):
        ...
        for j in range(len(preds[i]['rois'])):
            (x1, y1, x2, y2) = preds[i]['rois'][j].detach().cpu().numpy().astype(np.int32)
