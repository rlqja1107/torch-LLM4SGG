...
class ATSSPostProcessor(torch.nn.Module):
    ...
    def forward(self, box_regression, centerness, anchors,
        ...
        for idx, (b, c, a) in enumerate(zip(box_regression, centerness, anchors)):
            ...
            sampled_boxes.append(
                self.forward_for_single_feature_map(b, c, a, o, t, d, positive_map, feats)
            )
    ...
    def select_over_all_levels(self, boxlists, sgg_mode=None, targets=None):
        ...
        for i in range(num_images):
            ...
            if sgg_mode in ['sgcls', 'predcls']:
                ...
                label_match = result.get_field('labels').unsqueeze(1) == img_target.get_field('labels').unsqueeze(0)
                ious[~label_match] = -1.
                result.get_field("scores")[:] = 1
                match_scores = ious + result.get_field("scores").unsqueeze(-1)
                ...
                if number_of_detections > self.fpn_post_nms_top_n > 0:
                    ...
                    keep = cls_scores >= max(0, image_thresh.item())
                    keep = torch.nonzero(keep).squeeze(1)
                    result = result[keep]
            results.append(result)
        return results
