...
def do_train(
    ...
    if cfg.MODEL.RWT:
        data_loader.dataset.reweighting_dict = data_loader.dataset.reweighting_dict.cuda()
    for iteration, (images, targets, idxs, positive_map, positive_map_eval, greenlight_map) in tqdm(enumerate(data_loader, start_iter)):
        ...
        if cfg.SOLVER.USE_AMP:
            with autocast():
                if len(captions) > 0:
                    loss_dict = model(images, targets, captions, positive_map, greenlight_map = greenlight_map, rwt_dict = data_loader.dataset.reweighting_dict)
                else:
        ...
        if val_data_loader and (iteration % checkpoint_period == 0 or iteration == max_iter) and iteration > cfg.SOLVER.VAL_MIN_ITERATION:
            if is_main_process():
                print("Evaluating")
            eval_result = 0.0
        ...
        if iteration % checkpoint_period == 0 and iteration > cfg.SOLVER.VAL_MIN_ITERATION:
