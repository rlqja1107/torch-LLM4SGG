...
class GLIPDemo(object):
    ...
    def run_ner(self, caption):
        ...
        for entity, label in zip(relevant_phrases, labels):
            for m in re.finditer(re.escape(entity), caption.lower()):
                to_append_item = [[m.start(), m.end()]]
                if to_append_item not in tokens_positive:
                    kept_entities.append(entity)
                    tokens_positive.append(to_append_item)
                    break

        self.entities = kept_entities
        self.entities_caption_span = tokens_positive
        assert len(self.entities) == len(tokens_positive)
    ...
    def compute_prediction(self, original_image, original_caption, custom_entity = None):
        ...
        # compute predictions
        with torch.no_grad():
            predictions = self.model(image_list, captions=[original_caption], positive_map=positive_map_label_to_token, rwt_dict=None)
